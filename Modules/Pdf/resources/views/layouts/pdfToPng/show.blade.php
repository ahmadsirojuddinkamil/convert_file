...
<div style="background-color: #F3F0EC">
    <div class="text-center container py-5 pl-5">
        ...
        <div>
            <form action="" method="POST" enctype="multipart/form-data" id="reply-pdf-to-png">
                @csrf

                <label id="select-primary" for="pdf-to-png" style="display: inline-block" class="btn btn-secondary text-white px-5 py-3 rounded-pill">Pilih Pdf File</label>
                <label id="select-dumy" style="display: none" class="btn btn-secondary text-white px-5 py-3 rounded-pill">Pilih Pdf File</label>

                <input id="pdf-to-png" type="file" name="file_pdf" style="display: none" accept="application/pdf" onchange="validateFilePdf(event, 'pdf')">
                ...
                @foreach ($pngFiles as $png)
                    <li>
                        <ul>
                            ...
                            <a href="/pdf-to-png/{{ $png->uuid }}/download"
